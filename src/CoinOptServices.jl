...
type OsilMathProgModel <: AbstractMathProgModel
    ...
    linearConstraintCoefficients::XMLElement
    quadraticCoefficients::XMLElement
    nonlinearExpressions::XMLElement
...
function create_osil_common!(m::OsilMathProgModel, xl, xu, cl, cu, objsense)
    ...
    m.variables = new_child(instanceData, "variables")
    ...
    m.constraints = new_child(instanceData, "constraints")
    ...
    m.linearConstraintCoefficients = new_child(instanceData,
        "linearConstraintCoefficients")
    m.quadraticCoefficients = new_child(instanceData,
        "quadraticCoefficients")
    add_text(m.quadraticCoefficients, "")
    set_attribute(m.quadraticCoefficients, "numberOfQuadraticTerms", "0")
    m.numQuadConstr = 0 # move this once MathProgBase.loadquadproblem! exists
    m.nonlinearExpressions = new_child(instanceData, "nonlinearExpressions")
...
function MathProgBase.loadproblem!(m::OsilMathProgModel,
    ...
    if length(nzval) > 0
        ...
        set_attribute(m.linearConstraintCoefficients,
            "numberOfValues", length(nzval))
...
function MathProgBase.loadnonlinearproblem!(m::OsilMathProgModel,
    ...
    if numberOfNonlinearExpressions > 0
        ...
        set_attribute(m.nonlinearExpressions,
            "numberOfNonlinearExpressions", numberOfNonlinearExpressions)
        if nlobj
            nl = new_child(m.nonlinearExpressions, "nl")
